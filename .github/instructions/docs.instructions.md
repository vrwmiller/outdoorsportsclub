...
| Payment processor | **Stripe Terminal SDK** — Tap to Pay via tablet NFC (primary); **Stripe Reader M2** (Bluetooth hardware reader) is optional — covers chip/swipe fallback for members without a contactless card or mobile wallet. NFC is sufficiently ubiquitous that the M2 is not required at launch; it may be added if chip-only members prove common in practice. **Stripe.js** for online dues via **Member Portal** (card element, no NFC hardware required). |
...
| Waiver capture | **Kiosk View only** — waiver signing is not available on the **Member Portal**. Member signs on-screen via `signature_pad` (finger or S Pen on the **Samsung Galaxy Tab Active5**); PDF stored directly in **Amazon S3** with **S3 Object Lock** (Compliance Mode, 7-year retention). S3 object key written to `activity_logs.waiver_s3_key` for retrieval. See `docs/design.md` Section 5.5 and Section 7.2. |
...
* **If an ODQ's cleanup or resolution steps include a sequence of CLI commands**, extract those steps to a runbook under `docs/runbooks/` and reference it from the ODQ. The ODQ table should describe the required outcome; the runbook provides the procedure. Inline CLI commands (`aws ...`, `grep ...`, multi-step shell sequences) do not belong in the ODQ table.
