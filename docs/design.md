...
| 18 | **Public website content management** | Pre-launch | Club leadership and committees need to post news, announcements, and general club information without developer involvement. Options: (a) **Static MDX in the repo** — simple, no CMS dependency, but requires a developer to publish updates; (b) **Headless CMS** (e.g. Contentful, Sanity) — non-technical editors manage content independently, deployed on change via Amplify; (c) **Admin Portal integration** — Level 5+ staff manage content through the existing portal, stored in Aurora, rendered server-side. Decision affects frontend architecture, content ownership model, and whether any new AWS resources are needed. |
...
| 24 | **RSO shift calendar integration** | Post-launch | An RSO shift schedule is currently maintained in Google Calendar. It should be surfaced within the **Admin Portal** for RSOs and Administrators only — it is not exposed to general members. Decisions needed: (a) **Read access level** — whether the calendar view is gated at Level 4+ (RSO and above) or Level 5+ (Administrator and above); the current Google Calendar is accessible to RSOs only and it is unclear whether club Administrators currently have access; (b) **Edit/manage access** — whether RSOs update the schedule directly in Google Calendar (outside the app) with the application consuming it read-only via the Google Calendar API, or whether shift management moves into the **Admin Portal** with optional Google Calendar sync; (c) **Credential management** — the Google Calendar is private; an API key or service account credential must be stored in **AWS Secrets Manager** and must not be exposed to the browser. The minimum-scope implementation is a read-only Google Calendar API embed in the **Admin Portal** gated at Level 4+, with Google Calendar remaining the system of record for edits. |
| 25 | **Incomplete check-out mitigations** | Post-launch | Once members are conditioned to the QR check-in/check-out workflow, two RSO-assisted mitigations should be considered to reduce lanes stuck in an occupied state after a member walks off without signing out: (a) **Auto-checkout on range close** — when an RSO marks a range closed via `PATCH /v1/admin/ranges/{range_id}/status`, any lanes still showing occupied are automatically checked out with a `forced_close` flag written to `activity_logs`; (b) **RSO-initiated lane checkout** — a tap action in the RSO Dashboard allows the RSO to manually check out a specific lane on behalf of a member who has left. Both mitigations are additive and require no new infrastructure. Decisions needed: whether to implement one, both, or neither; and whether `forced_close` events should trigger any member notification. Defer until checkout compliance patterns are established post-launch. |
