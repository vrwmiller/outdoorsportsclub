...
| 28 | **Deployment environment model and promotion workflow** | Pre-launch | The environment model and promotion workflow must be defined before infrastructure implementation begins, as it determines how CloudFormation stacks are named and parameterised and whether a single AWS account or multiple accounts are used. Proposed three-environment model: (a) **dev** — feature branch preview environments spun up automatically by **AWS Amplify Gen 2** per branch; backend infrastructure deployed on demand by the developer; uses Stripe test keys and synthetic member data only; (b) **staging** — deployed automatically on merge to `main` via **GitHub Actions**; mirrors production configuration; used for integration testing and QA sign-off before promotion; (c) **prod** — deployed via **GitHub Actions** with a manual approval gate (**GitHub Actions environment protection rule**); triggered by manual promotion or a tagged release. Each environment owns its own CloudFormation stack, **Cognito** User Pool, **Aurora** cluster, and **Secrets Manager** secrets (Stripe test vs. live keys, DB credentials). Environment-specific CloudFormation parameters are stored in `infra/params/staging.json` and `infra/params/prod.json`. The GitHub Actions deployment sequence: lint + test on PR → deploy to staging on merge to `main` → run DB migrations → manual approval → deploy to prod. Decisions needed: (1) single AWS account with environment-namespaced stacks (simpler, lower cost) vs. separate AWS accounts per environment (stronger blast-radius isolation, higher overhead); (2) promotion trigger — manual tag vs. manual workflow dispatch; (3) whether staging shares the same AWS account as prod or is isolated; (4) Aurora ACU sizing per environment (dev/staging can use minimum 0.5 ACU; prod should be sized for expected load). This must be resolved before any CloudFormation stacks are written, as the account and naming strategy affects every resource. |
