...
| 17 | **RPO and restore path policy** | **~5 minutes RPO** — Aurora PITR is the primary restore path. The daily AWS Backup snapshot is retained as a fallback and for archival/compliance purposes. Aurora Global Database (zero RPO) is not provisioned and is out of scope. The kiosk offline event queue (ODQ #10) must be designed with this 5-minute window in mind. |
