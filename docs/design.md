...
| 28 | **Deployment environment model and promotion workflow** | Pre-launch | The "Deployment model" and "Non-production environment" sections (this document) already specify a two-environment model (`dev` and `prod` CloudFormation stacks), Aurora ACU sizing (0.5/2 for `dev`, 2/16 for `prod`), the `Environment: dev` CloudFormation parameter mechanic, and the requirement that the `dev` database never contain real member data. Those decisions are **not** open. The following are genuinely open: (1) **Single-account vs. multi-account** — whether `dev` and `prod` stacks share a single AWS account (simpler, lower cost, sufficient for a small club) or are deployed into separate AWS accounts (stronger blast-radius isolation, separate billing, separate Cognito pools and **AWS Secrets Manager** secrets without name-prefixing). This is the highest-priority decision because it determines IAM boundaries, **AWS Secrets Manager** secret naming (`osc/dev/…` vs. account-scoped), and CloudFormation stack naming conventions used in every template. (2) **Staging environment** — the existing design has no staging tier between `dev` and `prod`; whether a third `staging` environment (auto-promoted from `main` before a prod release) is needed must be decided before CloudFormation templates are written. Adding staging would require a third stack, a third **Cognito** User Pool, a third **Aurora** cluster, and a third set of **AWS Secrets Manager** secrets; if omitted, QA sign-off happens in `dev` before a manual `prod` promotion. (3) **CI/CD automation** — no GitHub Actions workflows exist yet; decisions needed: which events trigger staging deployment (merge to `main`, scheduled, or manual `workflow_dispatch`), which events trigger prod deployment (manual `workflow_dispatch` only, or tagged release), how DB migrations run in the pipeline, and whether `infra/params/<env>.json` CloudFormation parameter files or Amplify Gen 2 environment config is the authoritative parameterisation mechanism. |
