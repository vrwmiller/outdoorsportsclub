...
| 32 | **`sns:DestinationType: DirectPublish` condition on `KioskCheckOutRole`** | Pre-launch | The `sns:Publish` Allow statement on `KioskCheckOutRole` uses `Condition: StringEquals: sns:DestinationType: DirectPublish`. `DirectPublish` is not a documented runtime value for the `sns:DestinationType` IAM condition key — valid values are `Topic`, `PhoneNumber`, `Application`, `Lambda`, `SQS`, `HTTP`, and `firehose`. If this condition never matches at runtime, the Allow statement is permanently ineffective and the checkout Lambda has no working `sns:Publish` permission, silently dropping wait-list notifications. Decisions needed: (a) verify against AWS IAM condition key documentation whether `DirectPublish` is a valid value; (b) confirm how the checkout Lambda sends its notification (direct SMS to a member phone number — `PhoneNumber` — or via the `SNS_ALERTS_TOPIC_ARN` SNS topic — `Topic`); (c) update the condition in `infra/stacks/iam/lambda-kiosk-roles.yaml` to the correct value, and if `Topic`, scope the `Resource` to the specific topic ARN rather than `"*"`. Surfaced as a Medium finding during the pre-PR security review of PR #55. |
| 33 | **Orphaned `AuroraMasterSecret` cleanup** | Post-launch | `infra/stacks/secrets.yaml` retains an `AuroraMasterSecret` resource (`osc/<env>/aurora-master`) and its corresponding `osc-aurora-master-secret-arn-<env>` stack export. After PR #55 migrated the Aurora cluster to `ManageMasterUserPassword: true`, Aurora created its own managed secret; the old secret is orphaned and no stack imports its export. The resource has `DeletionPolicy: Retain`, so CloudFormation will not delete the underlying Secrets Manager secret automatically. Cleanup steps: (a) confirm no stack currently imports `osc-aurora-master-secret-arn-<env>` (`grep -r osc-aurora-master-secret-arn infra/`); (b) remove the `AuroraMasterSecret` resource and `AuroraMasterSecretArn` output from `secrets.yaml` and deploy; (c) after the stack update succeeds, manually delete the retained secret: `aws secretsmanager delete-secret --force-delete-without-recovery --secret-id osc/<env>/aurora-master --profile outdoorsportsclub`. The stale secret contains the former Aurora master password, which Aurora will have rotated away from after its first managed-password rotation cycle (within 7 days of the PR #55 deploy). |
