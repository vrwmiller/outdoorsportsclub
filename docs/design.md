...
| 27 | **Facility network infrastructure** | Deferred — a formal site survey has not been conducted. Network availability and capacity at the club's physical facilities are unconfirmed. Accepted as a known operational precondition outside this codebase's scope. No specific infrastructure approach has been selected; the options (wired Ethernet, point-to-point wireless, cellular LTE/5G) remain open. Kiosk hardware must not be deployed until connectivity at each range building is verified and deemed sufficient. The offline event queue (ODQ #10) provides resilience for brief outages but is not a substitute for absent infrastructure. |
