...
| :--- | :--- | :--- | :--- |
| 1 | **Waiver signing** | Pre-launch | What API endpoint handles waiver capture and signature? What is the payload (PDF blob, digital signature string, member acknowledgement)? Which surface captures it — Kiosk only, or also Member Portal on personal devices? |
...
| 23 | **Member-facing range occupancy view** | Post-launch | Members planning a visit have no way to gauge how busy a range is before making the trip. A range occupancy indicator (e.g. current vs. maximum lane count) derived from live `activity_logs` check-in data could be surfaced on the Member Portal without exposing camera feeds. Privacy considerations differ from camera access — occupancy counts do not identify individuals. Decisions needed: whether to surface occupancy data to members at all; what level of detail is appropriate (lane count, busy/moderate/quiet indicator, or nothing); whether historical trends (e.g. typical Saturday afternoon occupancy) add value; and how this interacts with the existing `GET /v1/admin/ranges/occupancy` endpoint (Level 4+) — a separate member-scoped endpoint with reduced detail may be needed. |
