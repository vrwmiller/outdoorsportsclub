...
| 27 | **Facility network infrastructure** | **Kiosk deployments restricted to verified-connectivity buildings.** Each clubhouse on the property is connected to the main clubhouse via fiber, with 1 Gbps internet terminating at the main clubhouse, and WiFi service is confirmed at the Rifle-Pistol range. Kiosk hardware must only be deployed to buildings where connectivity has been explicitly verified and deemed sufficient; wireless line-of-sight backup links are treated as optional redundancy rather than a prerequisite for kiosk deployment. The offline event queue (ODQ #10) is sized for brief outages only and must not be used to justify kiosk deployment at locations that lack stable primary connectivity. |
