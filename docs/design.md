...
| 33 | **Orphaned `AuroraMasterSecret` cleanup** | Deferred. Steps: confirm no stack imports `osc-aurora-master-secret-arn-<env>`, remove resource from `secrets.yaml`, deploy, delete the retained secret with `--recovery-window-in-days 7`. A runbook in `docs/runbooks/` must be written before execution. Tracked as Issue #81. |
