...
Updates the authenticated member's own editable profile fields. Accepted fields: `home_phone`, `mobile_phone`, `first_name`, `last_name`, `date_of_birth`, `street_address`, `city`, `state`, `zip`, `notification_email`, `notify_email`, `notify_sms`, `notify_push`. Validation rules (applied only when the field value is non-null): phone fields must be E.164 format; `state` must be exactly 2 letters (case-insensitive, stored uppercased); `zip` must be a non-empty string; `date_of_birth` must be a valid ISO 8601 date (`YYYY-MM-DD`); `notification_email` must be a valid email address. All text and date fields may be explicitly set to `null` to clear the stored value. Boolean fields (`notify_*`) do not accept `null`. Fields not present in the request body are left unchanged (partial update). `member_num`, `email`, `training_level`, `service_hours`, `dues_paid_until`, and `waiver_signed_at` are not updatable through this endpoint.
...
| :--- | :--- | :--- | :--- |
| 10 | **Offline operation architecture** | Pre-launch | [Issue #139](https://github.com/vrwmiller/outdoorsportsclub/issues/139) |
| 18 | **Public website content management** | Pre-launch | [Issue #140](https://github.com/vrwmiller/outdoorsportsclub/issues/140) |
| 21 | **Administrative reporting** | Post-launch | [Issue #134](https://github.com/vrwmiller/outdoorsportsclub/issues/134) |
| 22 | **Range camera access in Admin Portal** | Post-launch | [Issue #135](https://github.com/vrwmiller/outdoorsportsclub/issues/135) |
| 24 | **RSO shift calendar integration** | Post-launch | [Issue #136](https://github.com/vrwmiller/outdoorsportsclub/issues/136) |
| 26 | **Member credential hardware (NFC fob vs. QR-only)** | Pre-launch | [Issue #137](https://github.com/vrwmiller/outdoorsportsclub/issues/137) |
| 30 | **Dev venv Python version floor** | Pre-launch | [Issue #138](https://github.com/vrwmiller/outdoorsportsclub/issues/138) |

### Resolved

| # | Area | Decision |
| :--- | :--- | :--- |
| 34 | **Primary member record management — schema extension, name authority, and enrollment workflow** | (1) **Name authority:** Cognito `given_name`/`family_name` claims auto-populate `first_name`/`last_name` on first portal login if null; member-editable via `PATCH /v1/members/me`; Administrator can overwrite; last write wins. (2) **Membership type taxonomy:** Deferred — single member type for initial milestone; revisit before Junior/Family tiers are scoped. (3) **Renewal cadence:** Calendar-year (ODQ 2); `dues_paid_until = December 31` of payment year; existing column is sufficient. (4) **Required fields:** `street_address`, `city`, `state` (CHAR 2), `zip` for renewal mailing; `date_of_birth` nullable (reserved for future age-gated tiers). (5) **Enrollment workflow:** Member registers via Cognito first; fills contact record via Member Portal; Administrator may edit via Admin Portal endpoints; no pre-created member row required. (6) **Existing data migration:** Out of scope for this milestone. Schema adds `first_name`, `last_name`, `date_of_birth`, `street_address`, `city`, `state`, `zip`, `notification_email`, `notify_email`, `notify_sms`, `notify_push` to the `members` table. |
