...
| 18 | **Public website content management** | Pre-launch | Non-developer club members must be able to post news, announcements, and general club information without developer involvement — **static MDX in the repo is ruled out** as it requires a developer to publish every update. Remaining options: (a) **Headless CMS** (e.g. Contentful, Sanity) — non-technical editors manage content independently via a CMS UI, changes deploy automatically via Amplify; introduces a third-party dependency and ongoing cost; (b) **Admin Portal integration** — Level 5+ staff manage content through the existing portal, stored in Aurora, rendered server-side; no external dependency but adds scope to the Admin Portal build. Decisions needed: which option; and which role(s) or specific individuals will be responsible for content — this person has not yet been identified and must be consulted on usability requirements before a CMS tool is selected. |
| 19 | **Surface UI/UX design** | Pre-launch | The four application surfaces — **Club Website**, **Member Portal**, **Admin Portal**, and **Kiosk View** — require UI/UX design before frontend implementation begins. Each surface has distinct audiences and interaction patterns: the **Club Website** is public-facing and content-driven; the **Member Portal** is self-service on personal devices; the **Admin Portal** is a staff tool for range and membership management; the **Kiosk View** is a full-screen, RSO-operated workflow on a tablet. Design decisions include information architecture, navigation model, component library selection, and accessibility requirements. Wireframes or mockups for each surface should be produced and agreed upon before the corresponding `src/` implementation begins. |
| 20 | **Monitoring coverage gaps** | Pre-launch | Section 10 defines Lambda structured logging, three CloudWatch Alarms (Lambda error rate, API Gateway 5xx, Aurora ACU), and deferred X-Ray tracing. The following areas are not yet covered and must be defined before production launch: (a) **Frontend/Amplify** — client-side error tracking, Core Web Vitals, and JS exception reporting are not configured; options include AWS RUM or a third-party service; (b) **Aurora health** — no alarms exist for connection pool exhaustion, slow query volume, or replication lag beyond the ACU alarm; additional CloudWatch metric alarms should be defined; (c) **End-to-end availability** — no synthetic monitoring or uptime checks are defined; a CloudWatch Synthetics canary or equivalent should verify the Amplify endpoint and key API routes on a scheduled basis; (d) **Kiosk connectivity** — no alerting exists for kiosks that go offline unexpectedly or that accumulate a large offline event queue without flushing. Decisions on tooling, alert thresholds, and notification routing (SNS topic, on-call recipient) are needed. |
...
| 26 | **Member credential hardware (NFC fob vs. QR-only)** | Pre-launch | RSOs currently carry NFC fobs for facility access. There is unconfirmed discussion of extending NFC fobs to all members. This decision has direct consequences for kiosk hardware and the Member Portal credential model: (a) **QR-only (current design)** — the kiosk camera scans the member's QR badge from the **Member Portal** or a printed card; no NFC reader required on the tablet; the QR is the sole check-in credential; (b) **NFC fob for all members** — the kiosk tablet requires an NFC reader; fob provisioning becomes part of new-member onboarding (Admin Portal issues fob, fob ID recorded in the DB and linked to `member_id`); lost/stolen fob is deactivated by an Administrator; the **Member Portal** QR becomes a fallback credential (forgotten fob, new member awaiting fob delivery); (c) **Hybrid** — NFC fobs issued to RSOs and staff only (status quo); general members use QR exclusively. Decisions needed: confirm whether the club intends to issue fobs to all members; if yes, determine kiosk tablet NFC reader requirements (the **Samsung Galaxy Tab Active5** has a built-in NFC reader, so no additional hardware may be needed), define the fob provisioning and deactivation workflow in the Admin Portal, and decide the fallback policy when a member presents without their fob. This must be resolved before kiosk and Admin Portal implementation begins, as it affects both hardware procurement and the check-in API surface. |
| 27 | **Facility network infrastructure** | Pre-launch | The current state of network connectivity across Outdoor Sports Club's physical facilities is unknown. Internet access at the property has not been confirmed, and if it exists, its capacity and reliability relative to the demands of this system have not been assessed. This is an upstream blocker: **Stripe Terminal** (guest payments at the kiosk) requires a live connection to Stripe's API and has no offline path; the kiosk check-in flow depends on backend API calls on every scan; and the offline cache defined in ODQ #10 is a resilience mechanism for brief outages, not a substitute for absent infrastructure. If network infrastructure must be built, options to consider include: (a) **Wired Ethernet backbone** — conduit and CAT6/fiber runs between range buildings to a central equipment room with a single ISP uplink; most reliable and highest throughput; highest installation cost; (b) **Point-to-point wireless bridges** — outdoor wireless links (e.g., Ubiquiti airMAX) between buildings where trenching is impractical; lower installation cost but introduces weather and line-of-sight dependencies; (c) **Cellular LTE/5G failover or primary** — a 4G/5G router at each range building using a commercial data plan; lowest infrastructure cost and fastest to deploy; ongoing monthly cost and subject to carrier coverage at the property; suitable as a primary path if wired runs are not feasible, or as a failover for a wired backbone; (d) **Wi-Fi access points only** — consumer or business-grade APs distributing an existing uplink within buildings; insufficient on its own if no wired backhaul exists between buildings. Decisions needed: conduct a site survey to determine what connectivity (if any) currently exists at the property and whether it reaches each range building; assess whether existing service is sufficient in bandwidth and reliability for kiosk operations; if not, select and fund a network infrastructure approach before kiosk hardware is deployed. Note that even where internet access appears available (e.g., a staff office connection), it may not extend to range buildings or be suitable for production workloads. |
...
| 29 | **Stripe integration readiness** | Pre-launch | The decision to use **Stripe** for dues collection and guest payments is locked (see Section 12). Implementation is deliberately deferred pending club approval of the project proposal. Until the Webmaster explicitly lifts this hold, the following must not be implemented: (a) **Stripe secret** — `osc/dev/stripe-key` and `osc/prod/stripe-key` in **AWS Secrets Manager**; (b) **Dues payment endpoints** — `POST /v1/members/me/dues` (Stripe.js) and `POST /v1/kiosk/dues` (Stripe Terminal); (c) **Webhook handler** — `POST /v1/webhooks/stripe` (`payment_intent.succeeded`); (d) **Lambda IAM roles** for the above handlers; (e) **Kiosk guest payment step** in the check-in flow. Everything else in the system (infrastructure, auth, check-in/check-out, member management) may proceed. This ODQ is resolved only when the Webmaster confirms the club is ready to proceed with Stripe. |
| 31 | **CI/CD pipeline implementation timing** | Pre-launch | `infra/stacks/artifacts.yaml` and `infra/stacks/lambda.yaml` were added in PR #53. The deployment wiring (`Makefile`, `scripts/migrate.py`) is intentionally manual-first: the pipeline (`.github/workflows/deploy-dev.yml` and `deploy-prod.yml` per ODQ #28) has not been authored yet. The pipeline will be built once two preconditions are met: (a) the `dev` stack has been deployed at least once end-to-end from the Makefile (confirming the deploy order and parameter set are stable); and (b) the migration runner behaviour is validated in a live Aurora cluster. Implementing GitHub Actions workflow files before these preconditions are met would require rewriting them after every structural change to the stack or migration logic. The `cfn-lint` step defined in ODQ #28 may be added to CI independently as a fast lint-only job that requires no deployed AWS resources. |
| 32 | **`sns:DestinationType: DirectPublish` condition on `KioskCheckOutRole`** | Pre-launch | The checkout Lambda (`functions/kiosk/checkout/handler.py`) sends range wait-list notifications via **Amazon SNS** direct SMS using `sns.publish(PhoneNumber=...)`. For direct SMS, the IAM condition key `sns:DestinationType` must use value `PhoneNumber`; `DirectPublish` is not a documented runtime value for the `sns:DestinationType` IAM condition key — valid values are `Topic`, `PhoneNumber`, `Application`, `Lambda`, `SQS`, `HTTP`, and `firehose`. With the current `Condition: StringEquals: sns:DestinationType: DirectPublish`, the Allow statement on `KioskCheckOutRole` never matches and `sns:Publish` is effectively denied. When this happens, the checkout Lambda raises on publish and returns a 500 after the DB commit — the notification is skipped but the failure is not silent. Decision needed: update the condition in `infra/stacks/iam/lambda-kiosk-roles.yaml` to use `PhoneNumber` as the `sns:DestinationType` value and confirm no other IAM conditions rely on `DirectPublish`. Surfaced as a Medium finding during the pre-PR security review of PR #55. |
| 33 | **Orphaned `AuroraMasterSecret` cleanup** | Post-launch | `infra/stacks/secrets.yaml` retains an `AuroraMasterSecret` resource (`osc/<env>/aurora-master`) and its corresponding `osc-aurora-master-secret-arn-<env>` stack export. After PR #55 migrated the Aurora cluster to `ManageMasterUserPassword: true`, Aurora created its own managed secret; the old secret is orphaned and no stack imports its export. The resource has `DeletionPolicy: Retain`, so CloudFormation will not delete the underlying Secrets Manager secret automatically. Decisions needed: (a) confirm no stack currently imports `osc-aurora-master-secret-arn-<env>`; (b) remove the `AuroraMasterSecret` resource and `AuroraMasterSecretArn` output from `secrets.yaml` and deploy; (c) define and follow an operational runbook under `docs/runbooks/` that documents the safe deletion procedure for the retained Secrets Manager secret once the stack update succeeds (including verification steps and the exact CLI/API calls and flags). The stale secret contains the former Aurora master password, which Aurora will have rotated away from after its first managed-password rotation cycle (within 7 days of the PR #55 deploy). |
...
| 7 | **Pairing Code generation** | Admin Portal (Level 6) calls `POST /v1/admin/devices/pairing-code` to generate a 15-minute single-use code stored in `devices.pairing_code`. Tablet calls `POST /v1/devices/pair` to complete pairing. No unauthenticated code-request surface. See Section 5.3 and Section 7.3. |
