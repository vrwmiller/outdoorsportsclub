...
The application is a single **Next.js** app hosted on **AWS Amplify Gen 2**. The **Home Page** is the public-facing entry point visible to all visitors — it serves as **Outdoor Sports Club**'s primary web presence, with club information, membership information, and contact details managed by **Outdoor Sports Club** leadership and committees (see ODQ #18). After **AWS Cognito** login on a personal device, the nav menu expands based on the user's `training_level` fetched via a backend API call (**Lambda** re-queries **Aurora** via **RDS Data API**): members (Levels 1–3) see member-specific items and staff/admins (Levels 4–6) see additional management items. The **Kiosk View** is a dedicated full-screen route (`/kiosk`) within the same app, served to paired range tablets — it is accessed exclusively via Device Token and is entirely separate from the personal device login flow.
...
| `pairing_code` | TEXT (Nullable) | HMAC-SHA256 hash of the short-lived alphanumeric code generated by the Admin Portal during device provisioning. The raw code is returned to the Admin Portal exactly once and never stored. Single-use; cleared on successful pairing. Null for `Active` and `Revoked` devices. |
...
<tr><td style="background-color: #dbeafe; text-align: center; padding: 8px;"><strong>🗄️ Aurora Write Quorum</strong> — 6-node replication across 3 AZs; write confirmed by 4 of 6 nodes before ACK</td></tr>
<tr><td style="background-color: #c8dcfc; text-align: center; padding: 8px;"><strong>⚛️ Atomic Transactions</strong> — all multi-step DB ops execute in a single Aurora transaction; auto-rollback on Lambda failure</td></tr>
<tr><td style="background-color: #b5d0fa; text-align: center; padding: 8px;"><strong>🔄 Client-Side Retry</strong> — kiosk and portal retry on 5xx; all write endpoints are idempotent</td></tr>
<tr><td style="background-color: #a2c3f7; text-align: center; padding: 8px;"><strong>📴 Offline Event Queue</strong> — check-in/out events queued locally on connectivity loss; flushed automatically on reconnect</td></tr>
<tr><td style="background-color: #8fb5f4; text-align: center; padding: 8px;"><strong>💳 Stripe Webhook Durability</strong> — Stripe retries failed webhook delivery up to 72 h; handler idempotent on Payment Intent ID</td></tr>
<tr><td style="background-color: #7ca7f0; text-align: center; padding: 8px;"><strong>📡 Stripe Terminal Recovery</strong> — SDK recovers in-flight PaymentIntent after connectivity loss; payment never silently abandoned</td></tr>
</table>
...
**Returns:** For **Cash**: `200 OK` with `{ dues_paid_until }`. For **NFC** or **Card**: `202 Accepted` with `{ payment_intent_id }` (dues are confirmed and `dues_paid_until` set by the `payment_intent.succeeded` webhook). All paths: `402 Payment Required` (Stripe failure) or `404 Not Found` (unrecognised `member_num`).
...
The RPO target is **~5 minutes** — Aurora PITR is the primary restore path; the daily AWS Backup snapshot is the fallback of last resort. See ODQ #17 (resolved) in Section 11.
...
Each item requires a deliberate decision — do not implement with assumed behaviour.

| # | Area | Priority | Question |
| :--- | :--- | :--- | :--- |
| 10 | **Offline operation architecture** | Pre-launch | Member check-in and check-out must continue without internet connectivity. Proposed approach: the kiosk caches an encrypted local snapshot of active member QR tokens, `training_level`, and waiver/dues status at regular intervals while online. On connectivity loss, check-in validation runs against the local cache; events are queued and synced when connectivity restores. **Guest payment outage policy is resolved:** during a connectivity outage, the RSO may revert to paper-based methods if available, or grant provisional entry at their discretion — no payment record is required until the system is back online. The remaining open questions are technical: the caching strategy, encryption key management, and conflict-resolution on sync must be defined before kiosk implementation begins. |
| 18 | **Public website content management** | Pre-launch | Non-developer club members must be able to post news, announcements, and general club information without developer involvement — **static MDX in the repo is ruled out** as it requires a developer to publish every update. Remaining options: (a) **Headless CMS** (e.g. Contentful, Sanity) — non-technical editors manage content independently via a CMS UI, changes deploy automatically via Amplify; introduces a third-party dependency and ongoing cost; (b) **Admin Portal integration** — Level 5+ staff manage content through the existing portal, stored in Aurora, rendered server-side; no external dependency but adds scope to the Admin Portal build. **Interim resolution (2026-03-26):** The **Webmaster** (developer) is the content owner on an interim basis until a permanent club content owner is identified. Because the interim content owner is technical, the usability constraint that ruled out static MDX is relaxed for now — MDX in the repo is acceptable for the initial Home Page build. The CMS architecture decision (Headless CMS vs. Admin Portal integration) remains open and must be revisited before handing off content editing to non-technical club staff. Decision still needed: which option; the permanent content owner must be consulted on usability requirements before a CMS tool is selected. |
| 21 | **Administrative reporting** | Post-launch | Club management will need reports to support operational and financial oversight. No reporting layer is currently defined. Candidate report areas include: (a) **Membership** — active member count by training level, dues status (paid/lapsed/expiring), new member registrations over time, level promotions; (b) **Range operations** — check-in volume by range and date, peak occupancy periods, guest traffic, waiver capture rate; (c) **Finance** — dues revenue by period, guest fee revenue, consumable sales, payment method breakdown (Cash/NFC/Card), Stripe payout reconciliation; (d) **Compliance** — members with overdue waivers, members approaching annual guest limits, activity log export for audit purposes. Decisions needed: whether reports are built into the Admin Portal (Aurora queries via Lambda), exported to a BI tool, or delivered as scheduled email summaries. Scope and format of each report must be defined before the Admin Portal implementation begins. |
| 22 | **Range camera access in Admin Portal** | Post-launch | At least one range has installed cameras. Camera feeds may have operational value in the Admin Portal — allowing staff to monitor range activity remotely, review incidents, and reduce the need for physical walkthroughs. Access must be gated to a staff training level (exact level TBD — likely Level 4+ RSO or Level 5+ Administrator). Decisions needed: which camera system is installed and what integration mechanism it exposes (RTSP stream, vendor API, embedded iframe); how feeds are proxied or embedded without exposing stream credentials to the browser; whether recording/playback is in scope or live view only; and which training level(s) may access the feed. Camera access is not available to members. |
| 24 | **RSO shift calendar integration** | Post-launch | The club and its ranges use multiple Google Calendars for RSO shift scheduling. **Read access is confirmed at Level 4+** (RSO and above). Remaining decisions needed: (a) **Multi-calendar aggregation** — whether the Admin Portal surfaces a single primary calendar or aggregates multiple range-specific calendars into a unified view; aggregation has operational value given the multi-calendar structure but requires identifying and credentialing each calendar; (b) **Edit/manage access** — whether RSOs continue to update schedules directly in Google Calendar with the application consuming them read-only via the Google Calendar API, or whether shift management moves into the **Admin Portal** with optional Google Calendar sync; (c) **Credential management** — all referenced calendars are private; API keys or a service account credential must be stored in **AWS Secrets Manager** and must not be exposed to the browser. The minimum-scope implementation is a read-only Google Calendar API embed in the **Admin Portal** gated at Level 4+, with Google Calendar remaining the system of record for edits. |
| 26 | **Member credential hardware (NFC fob vs. QR-only)** | Pre-launch | RSOs currently carry NFC fobs for facility access. Club discussion is ongoing; current indication (unconfirmed) is that **NFC fobs will be the primary credential for all members**, with the QR badge becoming a convenience fallback (forgotten fob, new member awaiting delivery). This decision has direct consequences for kiosk hardware, the Member Portal credential model, and security posture. The three credential paths are: (a) **QR-only (current design)** — the kiosk camera scans the member's QR badge from the **Member Portal** or a printed card; no NFC reader required on the tablet; the QR is the sole check-in credential; (b) **NFC fob for all members (indicated preference)** — the kiosk tablet requires an NFC reader; fob provisioning becomes part of new-member onboarding (Admin Portal issues fob, fob ID recorded in the DB and linked to `member_id`); lost/stolen fob is deactivated by an Administrator; the **Member Portal** QR becomes a fallback credential; (c) **Hybrid** — NFC fobs issued to RSOs and staff only (status quo); general members use QR exclusively. **Security note (SEC-17):** The current `GET /v1/members/me/badge` endpoint (implemented by `functions/members/me-badge/handler.py`) returns a permanent unsigned `member_num` as the QR badge token. If QR is the primary credential, this must be replaced with a short-lived HMAC-signed token (≤ 60-second TTL) before launch — a replay of a photographed QR code could allow training-level bypass if a kiosk is ever temporarily left unattended or if an RSO relies solely on the QR scan result without physically verifying the member's identity. If fobs become primary and QR is a fallback only, the threat window narrows significantly and SEC-17 drops from High to Low. **SEC-17 implementation is suspended pending resolution of this ODQ.** Decisions needed before implementation can proceed: (1) confirm whether fobs will be issued to all members or remain RSO/staff-only; (2) if fob-primary, determine the kiosk tablet NFC reader protocol (the **Samsung Galaxy Tab Active5** has a built-in NFC reader — confirm it matches the fob hardware's protocol); (3) define the fallback policy for members with neither a fob nor a phone capable of displaying a QR code (printed badge? RSO manual lookup?); (4) define the fob provisioning and deactivation workflow in the Admin Portal. This must be resolved before kiosk and Admin Portal implementation begins, as it affects both hardware procurement and the check-in API surface. |
| 30 | **Dev venv Python version floor** | Pre-launch | `env.sh` accepts Python ≥ 3.12 (`PYTHON=${PYTHON:-python3}` with a version check) rather than a hard pin to the Lambda runtime version (currently 3.12). This tradeoff prioritizes single-contributor flexibility — the sole current contributor uses Python 3.13 — over strict runtime parity. If the contributor count grows, two concerns arise: (a) **Divergence risk** — contributors on different Python versions may produce slightly different `ruff` output or Pylance import resolution if the venv grows beyond its current linting-only scope; (b) **Reproducibility** — a hard pin (e.g. `PYTHON=${PYTHON:-python3.12}`) guarantees every contributor uses the same interpreter, simplifying debugging of environment-specific issues. The floor-based approach is appropriate for a single-developer project; revisit and consider a hard pin when additional contributors join. |
| 36 | **Primary member record management — schema extension, name authority, and Admin Portal enrollment workflow** | Pre-launch | Aurora is the intended primary membership database, maintained via the Admin Portal. The current `members` table lacks fields required for primary record management: `first_name`, `last_name`, mailing address (`street_address`, `city`, `state`, `zip`), and supporting fields for renewal tracking. Before the schema is extended or Admin Portal enrollment workflows are built, the following decisions must be made: (1) **Name authority** — When a member authenticates via Cognito social login (Google/Facebook), the JWT contains `given_name` and `family_name` claims. Are these claims the canonical source for `first_name`/`last_name` in `members` (populated on first login), or does the Administrator set the legal name at enrollment independently of what the social provider reports? If both paths exist, which wins on conflict? (2) **Membership type taxonomy** — What member tiers exist (e.g. Individual, Family, Junior, Life, Honorary)? Does tier affect dues amount, renewal cadence, guest allowances, or minimum training level for range access? Are `Family` and `Junior` tiers linked by a parent → dependent relationship in the schema? (3) **Renewal cadence** — Is renewal anniversary-based (1 year from `join_date`) or calendar-year (all memberships expire Dec 31 regardless of join date)? This determines whether `join_date` must be stored separately from `created_at` and whether the existing `dues_paid_until DATE` column is sufficient. (4) **Required mailing fields** — Confirm which address fields are required for the annual renewal mailing and whether `date_of_birth` is required for age-gated membership tiers (e.g. Junior). (5) **Admin enrollment workflow** — Does an Administrator create the `members` row first (with address and membership type) and the new member then claims it at first login via email match, or does the member register first via Cognito and an administrator subsequently completes the contact record? (6) **Existing data migration** — Is there a current membership roster (spreadsheet or prior system) that must be imported? If so, what fields does it contain and what is the import mechanism? These decisions determine the schema migration, at least three new Admin Portal endpoints (`POST /v1/admin/members`, `PATCH /v1/admin/members/{id}`, `GET /v1/admin/members`), how `GET /v1/members/me` is extended, and the annual renewal mailing workflow. |

### Resolved

| # | Area | Decision |
| :--- | :--- | :--- |
| 32 | **`sns:DestinationType` on `KioskCheckOutRole`** | Fixed `DirectPublish` → `PhoneNumber` in `infra/stacks/iam/lambda-kiosk-roles.yaml`. The checkout Lambda uses `sns.publish(PhoneNumber=...)` (direct SMS); the correct `sns:DestinationType` condition value is `PhoneNumber`. `DirectPublish` is not a valid runtime value for this condition key and caused the Allow statement to never match, effectively denying `sns:Publish`. |
| 2 | **Dues payment** | Two paths: personal-device via Stripe.js (`POST /v1/members/me/dues`); kiosk via Stripe Terminal (`POST /v1/kiosk/dues`). Both set `dues_paid_until` to Dec 31 of payment year; confirmation handled exclusively by the `payment_intent.succeeded` webhook on both paths. See Section 7.1 and Section 7.2. |
| 3 | **Service hours logging** | Manually maintained by Administrator via `PATCH /v1/admin/members/{member_id}/service-hours` (Level 5+); promotion via a separate level-change call to keep a human in the loop. `Service-Hours-Update` in `activity_logs`. See Section 7.3. |
| 4 | **`training_level` promotion** | Manual Administrator action only. `PATCH /v1/admin/members/{member_id}/level` (Level 5+); change recorded in `activity_logs`. No automated rule, no async infra required. See Section 7.3. |
| 5 | **Range Open / Close** | `ranges` table added; `PATCH /v1/admin/ranges/{range_id}/status` sets `is_open` (Level 4+); soft close — RSO physically clears the range. Five ranges seeded: Rifle-Pistol, Skeet-Trap, Air-Rifle, Indoor-Archery, Outdoor-Archery (names provisional). See Section 5.2 and Section 7.2. |
| 6 | **Member Portal read endpoints** | `GET /v1/members/me` and `GET /v1/members/me/badge` added to Section 7.1. Both re-query Aurora on every request; no data read from JWT claims. |
| 7 | **Pairing Code generation** | Admin Portal (Level 6) calls `POST /v1/admin/devices/pairing-code` to generate a 15-minute single-use code; its HMAC-SHA256 hash (keyed with `DEVICE_TOKEN_SALT`) is stored in `devices.pairing_code`. The raw code is returned to the Admin Portal once and never persisted. Tablet calls `POST /v1/devices/pair` to complete pairing. No unauthenticated code-request surface. See Section 5.3 and Section 7.3. |
| 8 | **Guest Level 0 flow entry point** | Guests must be physically present with their sponsoring member; no independent guest entry path exists. The guest add-on step (after member lane assignment) is the exclusive entry point for first-visit waiver capture and payment. See Section 4 and Section 7.2. |
| 9 | **Real-time RSO check-in view** | Kiosk shows local range occupancy (re-fetched after each transaction + 30s poll). Admin Portal / mobile web adds a cross-range supervisory view via `GET /v1/admin/ranges/occupancy` (Level 4+), polled at 30s. No push mechanism required. See Section 7.3. |
| 11 | **Violation override flow** | RSO taps own NFC badge (Level ≥ 4); logs `actor_member_id`. PIN rejected: observable, shareable, non-attributed. Approve: exception recorded in `activity_logs`; Deny: lane remains `Available`. See Section 4. |
| 12 | **Lane configuration management** | Initial counts seeded in bootstrap migration (Rifle-Pistol: 17 lanes); future changes via `POST /v1/admin/lanes` and `PATCH /v1/admin/lanes/{lane_id}` without requiring a migration. See Section 5.4 and Section 7.2. |
...
| 25 | **Incomplete check-out mitigations** | RSO-initiated force-checkout only — the RSO Dashboard's existing force-checkout action (already specified in Section 4) is sufficient to handle lanes left occupied after a member vacates. Auto-checkout on range close is not implemented. No member notification is sent on forced checkouts. See Section 4. |
| 27 | **Facility network infrastructure** | Partially confirmed (2026-03-28). **Confirmed:** Each clubhouse on the property is connected to the main clubhouse via fiber; 1 Gbps internet terminates at the main clubhouse. **WiFi confirmed at rifle-pistol range** — a wireless Roku device is in use there, confirming functional WiFi service at that building. **Planned redundancy:** Wireless line-of-sight (LoS) backup devices are planned to provide connectivity if the fiber backbone is severed; not yet deployed. **Remaining open:** A per-building connectivity audit has not been completed for all range structures; the LoS backup path is not yet operational. **Kiosk deployment condition unchanged:** kiosk hardware must not be deployed at any building until connectivity at that specific building is verified and deemed sufficient. The offline event queue (ODQ #10) provides resilience for brief outages but is not a substitute for absent infrastructure. |
| 28 | **Deployment environment model and promotion workflow** | **Single account, two stacks, name-prefixed secrets.** `dev` and `prod` stacks share one AWS account; secrets use `osc/dev/…` and `osc/prod/…` prefixes — no cross-account IAM. No staging tier — QA sign-off occurs in `dev` before manual prod promotion. **CI/CD:** merge to `main` auto-deploys to `dev` via `deploy-dev.yml`; push of a `vX.Y.Z` tag deploys to `prod` via `deploy-prod.yml`. Each deployment runs in order: (1) `cloudformation deploy`, (2) all `db/migrations/*.sql` in filename order via RDS Data API, (3) `lambda update-function-code`. Any step failure halts the pipeline immediately; no auto-retry; human intervention required. The CI test suite must pass before any deployment runs. See `docs/runbooks/ci-deployment-failure.md` (blocked runbook — to be written when workflow files are authored). |
| 19 | **Surface UI/UX design** | **Resolved 2026-03-26.** Component library: **shadcn/ui** (Tailwind-native, zero-runtime dependency — components copied into `src/components/ui/`). Wireframe approach: **design-as-build** — the designer agent implements each screen directly in code; no separate Figma or wireframe phase. The Tailwind design tokens, routing rules, RBAC gating, and accessibility standards already defined in `designer.instructions.md` constitute the authoritative design spec. Weather widget on the **Home Page** uses the **Open-Meteo** API (free, no API key required); club coordinates are hard-coded in `src/config/location.ts` — zip code 20121 (Centreville, VA): `lat: 38.8263, lon: -77.4283`. No environment variable is required for weather; the widget is always functional in any deployment. Tracked as Issue #84. |
| 20 | **Monitoring coverage gaps** | Tracked as Issue #83. Four gaps to close before production launch: (a) frontend/Amplify error tracking (AWS RUM or equivalent); (b) Aurora health alarms beyond the ACU threshold; (c) end-to-end synthetic monitoring (CloudWatch Synthetics or equivalent); (d) kiosk connectivity and offline queue alerting. |
| 29 | **Stripe integration readiness** | On hold pending Webmaster approval. Must not implement: Stripe secrets, dues payment endpoints (`POST /v1/members/me/dues`, `POST /v1/kiosk/dues`), `payment_intent.succeeded` webhook, Lambda IAM roles, kiosk guest payment step. Everything else may proceed. Tracked as Issue #85. |
| 31 | **CI/CD pipeline implementation timing** | Deferred until two preconditions are met: (a) `dev` stack deployed end-to-end from the Makefile; (b) migration runner validated in a live Aurora cluster. A cfn-lint-only CI job may be added independently at any time. Tracked as Issue #82. |
| 33 | **Orphaned `AuroraMasterSecret` cleanup** | `AuroraMasterSecret` resource and `osc-aurora-master-secret-arn-<env>` export removed from `infra/stacks/secrets.yaml` (Issue #81). Runbook: `docs/runbooks/aurora-master-secret-cleanup.md`. Remaining manual steps: redeploy the updated secrets stack and delete the retained Secrets Manager secret (`osc/<env>/aurora-master`) as documented in the runbook. |

## 12. Locked Decisions

These decisions are final. They must not be reopened without a documented reason and explicit agreement from all active contributors. Implementations must conform to them without deviation.

| Decision | Detail |
| :--- | :--- |
| **Payment processor** | **Stripe** is the sole payment processor for dues collection and guest fees. No alternative processors will be evaluated or integrated. Implementation gated on Webmaster approval (ODQ #29). |
| **Database** | **Amazon Aurora Serverless v2 (PostgreSQL-compatible)** is the database. No alternative data stores will be introduced for primary member, range, or transaction data. |
...
| **Lambda logging format** | All Lambda log output is **structured JSON** emitted via `logger.info(json.dumps({...}))`. Required fields: `request_id`, `member_id`, `device_id`, `action`, `duration_ms`, `error` (exception class name on failure; `null` on success). Error paths emit the structured record from the `finally` block with `error_name` set before any early return; no supplemental `logger.warning(...)`, `logger.error(...)`, or `logger.exception(...)` calls with `%s` format strings are permitted. See Section 7. |
